...
### 2. Competence
Witnesses must have **high DEX** (reliability) and **relevant HEX** (domain experience) in areas being witnessed.

**Dual requirement:**
- **DEX ≥ 0.80:** Reliable witness with consistent track record
- **HEX in domain:** Actual experience in the domain being evaluated
...
**Requirements:**
- DEX ≥ 0.80 (configurable) - Reliable witness
- n_eff ≥ 50 (sufficient confidence) - Enough evidence
- HEX domain experience - Minimum 10 interactions in relevant domain(s)
- HEX confidence ≥ 0.75 - Stable performance in domain
...
**Requirements:**
- DEX ≥ 0.90 - Highly reliable
- n_eff ≥ 200 - Extensive evidence
- HEX domain experience ≥ 50 interactions in relevant domain
- HEX confidence ≥ 0.90 - Deep, stable expertise
- Arbitration experience (via HEX "arbitration" domain)
...
```python
def check_domain_expertise(agent_id, required_domains, min_experience=10, 
                          min_confidence=0.75, shared_ledger):
    """
    Check if witness has HEX experience in required domains
    
    Uses HEX (experience corpus) to verify domain expertise:
    - Experience count: Number of successful interactions in domain
    - Confidence: Stability of performance in domain
    - Recency: When domain was last practiced
    
    Args:
        agent_id: Witness candidate
        required_domains: List of domains that need expertise
        min_experience: Minimum interaction count per domain (default 10)
        min_confidence: Minimum confidence score per domain (default 0.75)
        shared_ledger: Ledger interface
    
    Returns:
        (bool, str, dict): (qualified, reason, domain_info)
    """
    # Get witness HEX corpus
    hex_corpus = shared_ledger.get_hex(agent_id)
    
    if not hex_corpus or not hex_corpus.get('experience'):
        return False, "No HEX experience corpus found", {}
    
    domain_info = {}
    
    for domain in required_domains:
        # Find domain experience entry
        domain_exp = next(
        ...
        if not domain_exp:
            return False, f"No experience in domain '{domain}'", domain_info
        
        # Check experience count
        if domain_exp['count'] < min_experience:
            return False, \
                f"Domain '{domain}' has {domain_exp['count']} interactions, need {min_experience}", \
                domain_info
        
        # Check confidence
        if domain_exp['confidence'] < min_confidence:
            return False, \
                f"Domain '{domain}' confidence {domain_exp['confidence']:.2f} below {min_confidence}", \
                domain_info
        
        # Check recency (warn if stale, but don't reject)
        from datetime import datetime
        last_updated = datetime.fromisoformat(
            domain_exp['last_updated'].replace('Z', '+00:00')
        )
        days_since = (datetime.now(last_updated.tzinfo) - last_updated).days
        
        domain_info[domain] = {
            'count': domain_exp['count'],
            'confidence': domain_exp['confidence'],
            'last_updated': domain_exp['last_updated'],
            'days_since_last': days_since,
            'status': 'fresh' if days_since < 90 else 'stale'
        }
    
    return True, "Qualified in all required domains", domain_info
```
...
def select_witnesses_sortition(session_id, required_count, min_dex, domains, 
                               ...
                               min_domain_experience=10, min_domain_confidence=0.75):
    """
    ...
    for agent_id in all_agents:
        ...
        # Check domain expertise (using HEX)
        if domains:
            is_qualified, reason, domain_info = check_domain_expertise(
                agent_id, domains, min_domain_experience, 
                min_domain_confidence, shared_ledger
            )
            if not is_qualified:
                continue
    ...
    # Stage 2: Calculate weights using DEX and HEX
    weighted_candidates = []
    ...
    for agent_id in eligible:
        ...
        # Base weight from global DEX (reliability)
        dex_score = dex['alpha'] / (dex['alpha'] + dex['beta'])
        confidence = dex['alpha'] + dex['beta']
        ...
        # HEX domain expertise bonus
        hex_bonus = 1.0
        if domains and hex_corpus and hex_corpus.get('experience'):
            domain_scores = []
            for domain in domains:
                ...
                if domain_exp:
                    # Composite HEX score: count × confidence
                    # Normalize count (cap at 100 interactions)
                    count_normalized = min(domain_exp['count'] / 100, 1.0)
                    hex_score = (count_normalized * 0.5 + 
                                domain_exp['confidence'] * 0.5)
                    domain_scores.append(hex_score)
            
            if domain_scores:
                avg_hex_score = sum(domain_scores) / len(domain_scores)
                # Bonus ranges from 1.0 (minimal) to 2.0 (expert)
                hex_bonus = 1.0 + avg_hex_score
        
        # Witness experience bonus (from witness-specific HEX domain)
        witness_exp = next(
            (e for e in hex_corpus.get('experience', []) 
             if e['domain'] == 'witnessing'),
            None
        )
        witness_bonus = 1.0
        if witness_exp:
            # Up to +50% for 100+ witness attestations
            witness_bonus = 1.0 + min(witness_exp['count'] / 200, 0.5)
        
        # Final weight: DEX (30%) + HEX domain (60%) + Witness experience (10%)
        weight = (
            dex_score * confidence_factor * 0.3 +
            hex_bonus * 0.6 +
            witness_bonus * 0.1
        )
...
def appoint_witnesses_direct(witness_ids, agent_a_id, agent_b_id, domains, 
                             min_dex, min_domain_experience, min_domain_confidence, 
                             shared_ledger):
    """
    ...
    for witness_id in witness_ids:
        ...
        # Check domain expertise (using HEX)
        if domains:
            qualified, reason, domain_info = check_domain_expertise(
                witness_id, domains, min_domain_experience, 
                min_domain_confidence, shared_ledger
            )
            if not qualified:
                reasons.append(f"{witness_id}: {reason}")
...
```json
{
  ...
  "metadata": {
    ...
    "witness_domain_hex": {
      "finance": {"count": 35, "confidence": 0.92}
    }
  }
}
```
...
def perform_witness_attestation(witness_id, session_id, witness_private_key, shared_ledger):
    ...
    attestation = {
        ...
        'metadata': {
            ...
            'witness_domain_hex': get_agent_domain_hex(witness_id, session['task_summary']['domains'], shared_ledger)
        }
    }
...
### Witness HEX Accumulation

Witnesses accumulate experience in two HEX domains:

**1. Task Domain** - The domain they witnessed (e.g., "finance", "legal")
- Increments with each witnessed session
- Builds domain-specific witnessing expertise
- Used for future witness selection in same domain

**2. "witnessing" Domain** - General witnessing capability
- Increments with every attestation
- Tracks overall witness experience
- Used for witness reliability assessment

**Example HEX update after witnessing:**
```python
def update_witness_hex(witness_id, session, shared_ledger):
    """
    Update witness HEX after attestation
    
    Called after attestation is completed and bond released
    """
    hex_corpus = shared_ledger.get_hex(witness_id)
    domains = session['task_summary']['domains']
    outcome = 0.9  # Witnesses that don't get slashed count as success
    
    # Update each task domain
    for domain in domains:
        domain_exp = next(
            (e for e in hex_corpus['experience'] 
             if e['domain'] == domain),
        ...
        if domain_exp:
            ...
            # Confidence increases with consistent witnessing
            domain_exp['confidence'] = min(
                domain_exp['confidence'] + 0.01,
                0.99
            )
        else:
            hex_corpus['experience'].append({
                'domain': domain,
                'count': 1,
                'confidence': 0.7,  # Start higher than fresh agent
                'last_updated': datetime.utcnow().isoformat() + 'Z'
            })
    
    # Update witnessing domain
    witnessing_exp = next(
        (e for e in hex_corpus['experience'] 
         if e['domain'] == 'witnessing'),
        None
    ...
    if witnessing_exp:
        witnessing_exp['count'] += 1
        witnessing_exp['last_updated'] = datetime.utcnow().isoformat() + 'Z'
        witnessing_exp['confidence'] = min(
            witnessing_exp['confidence'] + 0.01,
            0.99
        )
    else:
        hex_corpus['experience'].append({
    ...
    # Sign and publish
    hex_corpus['signature'] = sign(hex_corpus, witness_keypair)
    shared_ledger.publish_hex(witness_id, hex_corpus)
    
    # Record HEX update event
    hex_update_event = {
        'event_type': 'hex_update',
        'agent_id': witness_id,
        'session_id': session['session_id'],
        'domains_updated': domains + ['witnessing'],
        'context': 'witness_attestation',
        'timestamp': datetime.utcnow().isoformat() + 'Z',
        'signature': sign(hex_update_event, witness_keypair)
    }
    shared_ledger.publish_event(hex_update_event)
```

**This dual accumulation ensures:**
- Witnesses become specialists in domains they frequently evaluate
- General witnessing skill is tracked separately
- Future selections favor experienced domain specialists
- HEX corpus shows both breadth (witnessing) and depth (domain expertise)

**Example witness HEX corpus:**
```json
{
  "hex_id": "uuid",
  "aex_id": "did:aex:witness_alice",
  "experience": [
    {
      "domain": "legal.contract_review",
      "count": 45,
      "confidence": 0.91,
    ...
    },
    {
      ...
      "count": 22,
      "confidence": 0.84,
      "last_updated": "2026-02-04T10:15:00Z"
    },
    {
      "domain": "witnessing",
      "count": 120,
...
### Example 3: HEX-Based Witness Selection

**Scenario:** High-value legal contract review requiring domain experts
...
witness_requirements = {
    'count': 3,
    'min_dex': 0.85,
    'domains': ['legal.contract_review'],
    'min_domain_experience': 20,  # HEX count
    'min_domain_confidence': 0.80  # HEX confidence
}
```
...
candidates = [
    {
        'agent_id': 'did:aex:witness_alice',
        'dex': {'score': 0.92, 'n_eff': 120},
        'hex': {
            'legal.contract_review': {'count': 45, 'confidence': 0.91, 'last_updated': '2026-02-01'},
            'witnessing': {'count': 120, 'confidence': 0.89}
        }
    },
    {
        'agent_id': 'did:aex:witness_bob',
        'dex': {'score': 0.88, 'n_eff': 95},
        'hex': {
            'legal.contract_review': {'count': 15, 'confidence': 0.78, 'last_updated': '2026-01-15'},
            'witnessing': {'count': 80, 'confidence': 0.85}
        }
    },
    {
        'agent_id': 'did:aex:witness_carol',
        'dex': {'score': 0.89, 'n_eff': 150},
        'hex': {
            'legal.compliance': {'count': 60, 'confidence': 0.92, 'last_updated': '2026-02-03'},
            'witnessing': {'count': 150, 'confidence': 0.90}
        }
    },
    {
        'agent_id': 'did:aex:witness_david',
        'dex': {'score': 0.91, 'n_eff': 105},
        'hex': {
            'legal.contract_review': {'count': 28, 'confidence': 0.86, 'last_updated': '2026-01-28'},
            'witnessing': {'count': 95, 'confidence': 0.87}
        }
    }
]
```
...
Candidate: Alice
  DEX: 0.92 ✓ (meets 0.85)
  HEX domain: legal.contract_review
    Count: 45 ✓ (meets 20)
    Confidence: 0.91 ✓ (meets 0.80)
    Status: fresh (4 days ago)
  → QUALIFIED
  Weight: DEX(0.92×0.3) + HEX(1.68×0.6) + Witness(0.6×0.1) = 1.344

Candidate: Bob
  DEX: 0.88 ✓ (meets 0.85)
  HEX domain: legal.contract_review
    Count: 15 ✗ (below 20 minimum)
  → REJECTED (insufficient domain experience)

Candidate: Carol
  DEX: 0.89 ✓ (meets 0.85)
  HEX domain: legal.contract_review
    Not found ✗
  → REJECTED (no experience in required domain)
  (Note: Has legal.compliance but not contract_review specifically)

Candidate: David
  DEX: 0.91 ✓ (meets 0.85)
  HEX domain: legal.contract_review
    Count: 28 ✓ (meets 20)
    Confidence: 0.86 ✓ (meets 0.80)
    Status: fresh (8 days ago)
  → QUALIFIED
  Weight: DEX(0.91×0.3) + HEX(1.43×0.6) + Witness(0.475×0.1) = 1.179
```

**Step 4: Selection**
```
Qualified witnesses: 2 (Alice, David)
Required: 3

Options:
1. Lower min_domain_experience to 15 → Would qualify Bob
2. Accept partial domain match → Would qualify Carol (legal.compliance)
3. Expand search radius → Look for more candidates
4. Proceed with 2 witnesses instead of 3

Decision: Lower threshold to 15, re-run selection
```

**Final Selection:**
```
Selected: Alice (weight 1.344), David (weight 1.179), Bob (weight 0.876)

Result: 3 witnesses with legal contract expertise
- Alice: 45 reviews, expert level
- David: 28 reviews, experienced
- Bob: 15 reviews, qualified minimum
```

**Key Insights:**
1. **HEX prevents mismatches:** Carol has high DEX but wrong domain
2. **Experience depth matters:** Count requirement ensures actual practice
3. **Confidence signals quality:** Low confidence even with high count is flagged
4. **Recency is tracked:** Can prefer witnesses with recent domain activity
5. **Witness specialization visible:** HEX corpus shows what witnesses are good at

---

**Status:** AEX_WITNESS v1.0 complete and normative (HEX-integrated)  
